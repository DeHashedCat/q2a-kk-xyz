...
class KK_XYZ_Captcha
{
    const INPUT_NAME = 'kk_xyz_form-div';
    ...
    function option_default($option)
    {
        ...
        if ($option == 'antibotcaptcha_noise') {
            return 0;
        }

        if ($option == 'antibotcaptcha_interval') {
            return 19;
        }

        if ($option == 'antibotcaptcha_angle') {
            return 10;
        }
    ...
    function admin_form()
    {
        ...
        if (qa_clicked('antibotcaptcha_save_button')) {
            ...
            qa_opt('antibotcaptcha_noise', (int)qa_post_text('antibotcaptcha_noise_field'));
            qa_opt('antibotcaptcha_interval', (int)qa_post_text('antibotcaptcha_interval_field'));
            qa_opt('antibotcaptcha_angle', (int)qa_post_text('antibotcaptcha_angle_field'));
        ...
        $form = array(
            ...
            'fields' => array(
                ...
                'noise' => array(
                    'label' => 'Noise:',
                    'value' => qa_opt('antibotcaptcha_noise'),
                    'tags' => 'NAME="antibotcaptcha_noise_field" min="0" max="100"',
                    'type' => 'number',
                    'note' => 'Image distortion level (0 = no noise, 100 = maximum). Values >70 use blur.',
                ),
                'interval' => array(
                    'label' => 'Interval:',
                    'value' => qa_opt('antibotcaptcha_interval'),
                    'tags' => 'NAME="antibotcaptcha_interval_field" min="0" max="100"',
                    'type' => 'number',
                    'note' => 'Interval between the start position of characters.',
                ),
                'angle' => array(
                    'label' => 'Char angle:',
                    'value' => qa_opt('antibotcaptcha_angle'),
                    'tags' => 'NAME="antibotcaptcha_angle_field" min="0" max="100"',
                    'type' => 'number',
                    'note' => 'Maximum skew of the symbol to the left and right',
                )
    ...
    public function form_html(&$qa_content, $error)
    {
        ...
        $imagePage = qa_path_html('kk_xyz_page');
